...
class MetaPathGenerator:
    ...
    def read_utlpptludata(self, dirpath):
        self.id_poi.clear()
        self.id_user.clear()
        self.id_tl.clear()
        self.user_tllist.clear()
            ...
            for line in pafile:
                ...
                if len(toks) == 3:
                    ...
                    if tl not in self.tl_poilist:
                        self.tl_poilist[tl] = []
                    self.tl_poilist[tl].append(p)
        with open(dirpath + "\\poi_poi.txt",'r', encoding='ISO-8859-1') as pfile:
            for line in pfile:
                toks = line.strip().split("\t")
                if len(toks) == 2:
                    p1, p2 = toks[0], toks[1]
                    ...
                    if p2 not in self.poi_poilist:
                        self.poi_poilist[p2] = []
                    self.poi_poilist[p2].append(p1)
    def generate_random_utlpptlu(self, outfilename, numwalks, walklength):
        outfile = open(outfilename, 'w', encoding="ISO-8859-1")
        for user in self.user_tllist:
            user0 = user
            #随机初始tl0
            ...
            for j in range(0, numwalks):
                ...
                for i in range(0, walklength):
                    ...
                    if poi not in self.poi_poilist:
                        break
                    else:
                        poi2s = self.poi_poilist[poi]
                        numb = len(poi2s)
                        poiid = random.randrange(numb)
                        poi2 = poi2s[poiid]
                        outline += " " + self.id_poi[poi2]  # 路径U TL P P

                        if poi2 not in self.poi_tllist:
                            break
                        else:
                            tls = self.poi_tllist[poi2]
                            numtl = len(tls)
                            count = 0
                            while (1):
                                count = count +1
                            ...
                            userid = random.randrange(numu)
                            user = users[userid]
                            outline += " " + self.id_user[user]  #路径U TL P P TL U
                outfile.write(outline + "\n")
        outfile.close()
...
utlpptlu_dirpath = ".\\data\\utlpptlu\\input"
utlpptlu_outfilename = ".\\data\\utlpptlu\\vector\\random_walks.txt"
...
if __name__ == "__main__":
    ...
    mpg.read_utlpptludata(utlpptlu_dirpath)
    mpg.generate_random_utlpptlu(utlpptlu_outfilename, numwalks, walklength)
